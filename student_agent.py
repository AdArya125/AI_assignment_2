...
def convert_board_to_dict(board: List[List[Any]]) -> List[List[Dict[str, str]]]:
    converted = []
    for row in board:
        new_row = []
        for cell in row:
            if cell is None:
                # Represent empty cell as empty dictionary
                new_row.append(dict())
            else:
                cell=cell.to_dict()
...
class StudentAgent(BaseAgent):
    ...
    def choose(self, game_state: List[List[Any]],  rows: int, cols: int, score_cols: List[int], current_player_time: float, opponent_time: float) -> Optional[Dict[str, Any]]:
        board = convert_board_to_dict(game_state)
        cpp_move = self.agent.choose(board, score_cols, current_player_time, opponent_time)
        if cpp_move is None:
            return None
        ...
        # i=0
        # if(i==0):
        #     print("C++ Move:", cpp_move)
        #     print("Converted Move:", move_dict)
            # i=1
...
def test_student_agent():
    ...
    try:
        ...
        move = agent.choose(board, score_cols, 1.0, 1.0)
